...
from optimizers import GradientAccumulator
from utils import utils


class BasedTrainer(metaclass=abc.ABCMeta):
    """Customized trainer model for all models."""
    ...
    def reset_states_eval(self):
        """Reset eval metrics after saving it to tensorboard."""
    ...
    def _train_epoch(self):
        ...
        for train_steps_per_epoch, batch in enumerate(self.train_data_loader, 1):
            self._train_step(batch)  # one step training
    ...
    def _check_save_interval(self):
        ...
        if self.steps % self.config["save_interval_steps"] == 0:
            ...
            logging.info(f"Succesfully saved checkpoint @ {self.steps} steps.")
...
class GANBasedTrainer(BasedTrainer):
    """Customized trainer for MB-MelGAN model"""
    def __init__(self, steps, epochs, config, strategy,
                 is_generator_mixed_precision=False, is_discriminator_mixed_precision=False):
        """
        Initialize trainer.
        :param steps: (int) Initial global steps.
        :param epochs: (int) Initial global epochs.
        :param config: (dict) Config dict loaded from yaml format configuration file
        :param strategy: type of strategy to train the model
        :param is_generator_mixed_precision: (bool)
        :param is_discriminator_mixed_precision: (bool)
        """
    ...
    def set_gen_optimizer(self, generator_optimizer):
        ...
        if self._is_generator_mixed_precision:
            self._gen_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(self._gen_optimizer, "dynamic")
    ...
    def set_dis_optimizer(self, discriminator_optimizer):
        ...
        if self._is_discriminator_mixed_precision:
            self._dis_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(self._dis_optimizer, "dynamic")
    ...
    def compile(self, gen_model, dis_model, gen_optimizer, dis_optimizer):
        self.set_gen_optimizer(gen_model)
        self.set_dis_optimizer(dis_model)
    ...
    def compute_per_example_generator_losses(self, batch, outputs):
        ...
        :param batch: dictionary batch input return from dataloader
        :param outputs: outputs of the model
        :return: per_example_losses: per example losses for each GPU, shape [B]
    ...
    def compute_per_example_discriminator_losses(self, batch, gen_outputs):
        ...
        :param batch: dictionary batch input return from dataloader
        :param gen_outputs: outputs of the model
        :return:
    ...
    def _calculate_generator_gradient_per_batch(self, batch):
        ...
        per_replica_gen_losses = tf.nn.compute_average_loss(per_example_losses,global_batch_size=self.config["batch_size"]*self.get_n_gpus()*self.config["gradient_accumulation_steps"],)
    ...
    def _calculate_discriminator_gradient_per_batch(self, batch):
        (per_example_losses, dict_metrics_losses) = self.compute_per_example_discriminator_losses(batch, self._generator(**batch, training=True))
        per_replica_dis_losses = tf.nn.compute_average_loss(per_example_losses, global_batch_size=self.config["batch_size"] * self.get_n_gpus() * self.config["gradient_accumulation_steps"])

        if self._is_discriminator_mixed_precision:
            scaled_per_replica_dis_losses = self._dis_optimizer.get_scaled_loss(per_replica_dis_losses)

        if self._is_discriminator_mixed_precision:
            scaled_gradients = tf.gradients(scaled_per_replica_dis_losses,self._discriminator.trainable_variables)
        else:
    ...
    def _one_step_forward_per_replica(self, batch):
        ...
        if self.config["gradient_accumulation_steps"] == 1:
            (gradients, per_replica_dis_losses) = self._calculate_generator_gradient_per_batch(batch)
            ...
            for i in tf.range(self.config["gradient_accumulation_steps"]):
                reduced_batch = {k: v[i * self.config["batch_size"] : (i + 1) * self.config["batch_size"]]
    ...
    def create_checkpoint_manager(self, saved_path=None, max_to_keep=10):
        ...
        self.ckpt = tf.train.Checkpoint(steps=tf.Variable(1), epochs=tf.Variable(1),
    ...
    def save_checkpoint(self):
        ...
        utils.save_weights(self._discriminator, self.saved_path + "discriminator-{}.h5".format(self.steps))
    ...
    def load_checkpoint(self, pretrained_path):
        ...
        utils.load_weights(self._discriminator, self.saved_path + "discriminator-{}.h5".format(self.steps))

    def _check_train_finish(self):
        """Check Training finished or not."""
        if self.steps >= self.config["train_max_steps"]:
            self.finish_train = True

        if (self.steps != 0 and self.steps == self.config["discriminator_train_start_steps"]):
            self.finish_train = True
            logging.info(f"Finished training only generator at {self.steps} steps, Please resume and continue training.")

    def _check_log_interval(self):
        """Log to tesorboard."""
        if self.steps % self.config["log_interval_steps"] == 0:
            for metric_name in self.list_metrics_name:
                logging.info(f"(step: {self.steps}) train_{metric_name} = {self.train_metrics[metric_name].result():.4f}")
